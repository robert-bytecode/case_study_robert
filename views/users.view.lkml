view: users {
  ...
  dimension: customer_length {
    type: string
    sql: CASE
          WHEN ${created_raw} >= TIMESTAMP(DATETIME_ADD(CURRENT_TIMESTAMP(), INTERVAL -89 DAY)) AND ${created_raw} < TIMESTAMP(DATETIME_ADD(CURRENT_TIMESTAMP(), INTERVAL 89 DAY))
          THEN "New Customer"
  ...
  measure: older_user_count {
    type: count
    filters: [is_new_user: "Returning User"]
  ...
  set: detail {
    fields: [
      id, last_name, first_name, email, street_address, city, state, postal_code, country, gender, age
    ]
  }

  set: product_detail {
    fields: [
      products.id, products.sku, products.name, products.category, products.department, products.brand
    ]
  }
}
